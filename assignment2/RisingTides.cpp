...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> grid;
    grid.resize(terrain.numRows(),terrain.numCols(),false);
    for(GridLocation location:sources){
        if(terrain[location.row][location.col]<=height){
            grid[location.row][location.col]=true;
            GridLocation tmp={location.row,location.col};
            queue.enqueue(tmp);
        }
    }
    while(!queue.isEmpty()){
        GridLocation tmp=queue.dequeue();
        int row=tmp.row;
        int col=tmp.col;
        if(terrain.inBounds(row-1,col) && grid[row-1][col]==false && terrain[row-1][col]<=height){
            grid[row-1][col]=true;
            GridLocation newLocation={row-1,col};
        ...
        if(terrain.inBounds(row,col-1) && grid[row][col-1]==false && terrain[row][col-1]<=height){
            grid[row][col-1]=true;
            GridLocation newLocation={row,col-1};
            queue.enqueue(newLocation);
        }
        if(terrain.inBounds(row,col+1) && grid[row][col+1]==false && terrain[row][col+1]<=height){
            grid[row][col+1]=true;
