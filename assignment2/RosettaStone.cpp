...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for(Corpus tmp:corpora){
        double similarity=cosineSimilarityOf(textProfile,tmp.profile);
        if(similarity>max){
            language=tmp.name;
            max=similarity;
        }
